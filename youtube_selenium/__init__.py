...
class YouTubeUploader:
    ...
    def __init__(
        self, video_path: str, metadata_dict: Optional[defaultdict] = None
    ) -> None:
    ...
    def __upload(self) -> Tuple[bool, Optional[str]]:
        ...
        if self.metadata_dict["thumbnail_path"]:
            absolute_thumbnail_path = str(
                Path.cwd() / self.metadata_dict["thumbnail_path"]
            )
            ...
            self.logger.debug(
                "Attached thumbnail {}".format(self.metadata_dict["thumbnail_path"])
            )

        title_field = self.browser.find(By.XPATH, Constant.TITLE, timeout=10)
        self.__write_in_field(
            title_field, self.metadata_dict[Constant.VIDEO_TITLE], clear=True
        )
        self.logger.debug(
        ...
        if video_description:
            ...
            self.__write_in_field(
                description_field, self.metadata_dict[Constant.VIDEO_DESCRIPTION]
            )
            self.logger.debug(
        ...
        self.__write_in_field(
            tags_field, self.metadata_dict[Constant.VIDEO_TAGS], clear=True
        )
        self.logger.debug(
